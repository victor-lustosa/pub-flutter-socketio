...
class ParticipantViewModel extends ChangeNotifier implements IParticipantViewModel{
  ...
  Alignment alignment(state,index){
    if(state is SendMessageState || state is ReceivePublicMessageState) {
      if(_participant.getMessages[index].getUser.getNickname != _user.getNickname){
  ...
  typeMessage(state, index) {
    ...
    // });
    if(state is SendMessageState || state is ReceivePublicMessageState) {
      return Text('${_participant.getMessages[index].getUser.getNickname} - ${_participant.getMessages[index].getTextMessage}');
